...
int main(int argc, char *argv[])
{
  ...
  float poiss_rate = 2000.0; // poisson signal rate in Hz
  float poiss_weight = 0.369;
  float poiss_delay = 0.2; // poisson signal delay in ms
  int n_pg = 100; // number of poisson generators
  ...
  neural_gpu.ConnectFixedIndegree(pg, n_pg, neuron, n_neurons, 0, poiss_weight,
				  poiss_delay, 10);
