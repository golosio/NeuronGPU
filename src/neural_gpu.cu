...
/*
	uint myarr[4];
	gpuErrchk(cudaMemcpy(myarr, d_SpikeTargetNum,
			     4*sizeof(uint), cudaMemcpyDeviceToHost));
	cout << "myarr " << myarr[0] << " "  << myarr[1] << " " 
	     << myarr[2] << " "  << myarr[3] << endl;
	////////////////////////////////
	cout << "n_get_spikes: " << n_get_spikes << endl;
*/
	time_mark = getRealTime();
	uint grid_dim_x, grid_dim_y;
	if (n_get_spikes<65536*1024) { // max grid dim * max block dim
	  //cout << "ok " << (n_get_spikes+1023)/1024 << endl;
	...
	//cout << "ok " << grid_dim_x << " " << grid_dim_y << endl;
