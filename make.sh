...
nvcc -ccbin=mpicc --compiler-options -Wall --compiler-options '-fPIC' --compiler-options '-fopenmp' -arch sm_30 --ptxas-options=-v --maxrregcount=55 --relocatable-device-code true -I $CUDA_SAMPLES//6_Advanced/scan/ -I $CUDA_SAMPLES/common/inc/ -I $CUB -I $CUB/test --shared -o ../lib/libneuralgpu.so neuron_models.cu base_neuron.cu neural_gpu.cu aeif.cu connect.cu connect_mpi.cu poisson.cu rk5.cu spike_buffer.cu send_spike.cu get_spike.cu spike_mpi.cu getRealTime.cu spike_generator.cu multimeter.cu random.cu nested_loop.cu prefix_scan.cu scan_tmp.cu neuron_group.cu -lm -lstdc++ -lcurand
